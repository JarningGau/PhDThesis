---
title: "Analysis of Zfp541 KO data"
author: "Jarning"
date: "2022/4/28"
...
```{r, eval=FALSE}
RunCNMF(counts.fn = "mTCA_data/mTCA_metacell.counts.h5ad", 
        run.name = "K70", 
        genes.fn = "mTCA_data/HVGs.2000.all.txt", 
        out.path = OUTPATH, 
        K = 70, 
        n.iter = 50, 
        cores = 20, 
...
We can also train an ensemble SVM classifiers via `FitEnsemblMultiClassif()` function, although we do not use it here.
...
## inputs
seu <- readRDS("example_dataset/query.Zfp541KO_vs_WT.rds")
seu$genotype <- ifelse(seu$orig.ident == "WT", "WT", "Zfp541-KO")
...
DimPlot(seu, reduction = "umap_pred", group.by = "annotation", split.by = "genotype", label = F, pt.size = .1) +
  ggsci::scale_color_d3("category20")
DimPlot(seu, reduction = "umap", group.by = "annotation", split.by = "genotype", label = F, pt.size = .1) +
  ggsci::scale_color_d3("category20")
VlnPlot(seu, features = "pseudotime.pred", group.by = "annotation", pt.size = 0)
```
...
p1 <- DimPlot(seu, reduction = "umap_pred", group.by = "annotation") + ggsci::scale_color_d3("category20")
...
```{r, eval=FALSE}
saveRDS(seu, file.path(OUTPATH, "output.Zfp541KO_vs_WT.rds"))
```
...
celltype.levels <- c("Spermatogonia", "pre Leptotene", "Leptotene", "Zygotene", "Pachytene", "Diplotene", "Meiotic metaphase", "Round spermatids", "Elongated spermatids")
...
test_cum <- query.plot %>% subset(genotype == "Zfp541-KO") %>% cal_cum()
...
data.plot %>% 
  ...
  geom_jitter(inherit.aes = F, data = subset(data.plot, genotype == 'WT'),
              ...
              height = 0.05, size = .1, alpha = 1, show.legend = F) +
  geom_jitter(inherit.aes = F, data = subset(data.plot, genotype == 'Zfp541-KO'),
...
saveRDS(seu.clean, file.path(OUTPATH, "output.Zfp541KO_vs_WT.clean.rds"))
```
...
```{r}
OUTPATH <- "case1_data"
seu <- readRDS(file.path(OUTPATH, "output.Zfp541KO_vs_WT.clean.rds"))
...
# merge data
seu.magic <- merge(seu.list$WT, seu.list$`Zfp541-KO`)
...
```{r}
seu.magic$pseudotime.bin <- infotheo::discretize(seu.magic$pseudotime.pred, disc = "equalwidth", 100)[[1]]
...
data.label <- lmm.res %>% subset(genename %in% c("Gm960","Gm4969","Mei1","Zcwpw1"))
data.label2 <- lmm.res %>% subset(genename %in% c("Rad51ap2", "Stra8","Sycp2","Piwil1"))
...
es.df <- readRDS("case1_data/global_analysis.expression_shift.rds")
...
data.use$pseudotime.bin <- factor(data.use$pseudotime.bin, levels = 1:100)
data.use <- data.use %>% 
...
es.df <- es.df %>% 
  mutate(pseudotime.bin = factor(pseudotime.bin, levels = 1:100))
...
es.df %>% 
  ...
  xlab("Pseudotime") + ylab("Expression shift") + ggtitle("Zfp541-KO vs WT") + 
  scale_x_discrete(labels = axis.labels[as.character(unique(es.df$pseudotime.bin))]) + 
  ggsci::scale_fill_d3() + 
  theme_bw(base_size = 15) + 
...
```{r, fig.width=6, fig.height=4}
## Zfp541 expression
data.use <- FetchData(seu.magic, vars = c("pseudotime.bin", "Zfp541", "genotype"))
ggplot(data.use, aes(factor(pseudotime.bin), Zfp541)) + 
  geom_boxplot(aes(color = genotype), outlier.shape = NA) +
  scale_x_discrete(breaks = seq(5,100,5) %>% as.character(), labels = axis.labels[seq(5,100,5) %>% as.character()]) + 
  xlab("Pseudotime") + ylab("Zfp541 (imputed)") + 
  theme_bw(base_size = 15) + 
  theme(axis.text = element_text(color = "black"),
        plot.title = element_text(hjust = .5),
        axis.text.x = element_text(angle = 45, hjust = 1),
...
ggsave(file.path(OUTPATH, "plots/global_analysis.Zfp541_imputed_expression.png"), width = 6, height = 4, units = "in", dpi = 600)
```
...
input.mat.2 <- subset(data.use, genotype == "Zfp541-KO") %>% select_if(~ !any(is.na(.)))
...
csiMat <- Gadest::CalculateCSIMatrix(pccMat, cores = 10)
