...
OUTDIR <- here("results/07.label_transfer.mouse")
safe_mkdir(OUTDIR)

#### 1. build dataset ####
dataset <- readRDS(here("results/06.cNMF_mouse/mTCA_mc.all.cnmf.hvg2000/K70/mTCA.AUCell.rds"))

files <- list.files(here("results/03.manual_annotation/"), full.names = T)
files <- files[grepl("*mouse*", files)]
...
## from ~100,000 down to ~35,000 cells
dataset.labeled.ds <- downsample(dataset.labeled, batch = "Cell_type_manual", min.size = 2000)

saveRDS(dataset.labeled, file.path(OUTDIR, "mTCA.AUCell.labeled.rds"))
saveRDS(dataset.labeled.ds, file.path(OUTDIR, "mTCA.AUCell.labeled.ds.rds"))
saveRDS(dataset.unlabeled, file.path(OUTDIR, "mTCA.AUCell.unlabeled.rds"))

#### 2. model benchmark ####
dataset.labeled <- readRDS(file.path(OUTDIR, "mTCA.AUCell.labeled.ds.rds"))

task = as_task_classif(dataset.labeled, target = "Cell_type_manual", id = "mTCA")
...
#### 3. label transfer ####
dataset.labeled.ds <- readRDS(file.path(OUTDIR, "mTCA.AUCell.labeled.ds.rds"))
dataset.labeled <- readRDS(file.path(OUTDIR, "mTCA.AUCell.labeled.rds"))
dataset.unlabeled <- readRDS(file.path(OUTDIR, "mTCA.AUCell.unlabeled.rds"))
dataset <- rbind(dataset.labeled[-ncol(dataset.labeled)], dataset.unlabeled)
task = as_task_classif(dataset.labeled.ds, target = "Cell_type_manual", id = "mTCA")
...
saveRDS(learner, file.path(OUTDIR, "mTCA.manual.SVM.rds"))
saveRDS(dataset[, c("pred.label", "purity")], file.path(OUTDIR, "mTCA.predicted_labels.rds"))
