...
K = 70
run.name = paste0("K", K)
INDIR = here("results/06.cNMF_mouse/mTCA_mc.all.cnmf.hvg2000")


#### geneset analysis ####
top.genes <- readr::read_tsv(file.path(INDIR, run.name, paste0("top100_genes.k_",K,".dt_0_2.txt")), show_col_types = FALSE)
term2gene <- readRDS(here("data/gene_set/MCA_TableS4_full.mouse_symbol.rds"))
head(term2gene)
...
aucMat <- readRDS(file.path(INDIR, run.name, "mTCA.AUCell.rds"))
## cell type matrix
