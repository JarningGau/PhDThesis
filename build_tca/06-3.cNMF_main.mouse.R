...
INFILE = here("results/06.cNMF_mouse/mTCA_metacell.counts.h5ad")
HVGFILE = here("results/06.cNMF_mouse/HVGs.2000.all.txt")
OUTPATH = here("results/06.cNMF_mouse/mTCA_mc.all.cnmf.hvg2000")
...
if (T) {
  ...
  seu <- sceasy::convertFormat(here("results/02.preprocess_mouse/TCA_mouse.processed.h5ad"),
  ...
  saveRDS(newdata, file.path(OUTPATH, run.name, "mTCA.AUCell.rds"))

  ## plots
  newdata <- readRDS(file.path(OUTPATH, run.name, "mTCA.AUCell.rds"))
  umap.emb <- read.csv(here("results/04.scvi/TCA_mouse.emb_scvi.csv"), row.names = 1)
